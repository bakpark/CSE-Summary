...
> **Agentic Work Platform은 현업 사용자의 업무 설명을 실행 가능한 에이전트로 만들고, 개발환경 없이도 안전하게 빌드·검증·배포·운영할 수 있게 하는 AI 업무 실행 플랫폼이다.**

이 플랫폼의 6주 핵심 가치는 “자연어로 코드를 생성한다”가 아니다.

> **비개발자가 GitHub와 로컬 개발환경을 직접 다루지 않아도, 회사가 관리하는 도구와 실행 환경 안에서 에이전트를 만들고 검증된 버전으로 운영할 수 있게 한다.**

### 1.5 비전

> **개인의 일하는 방식을, 회사가 운영할 수 있는 AI 업무로.**

CCAB는 사용자의 자연어와 업무 지식을 에이전트 코드·설정으로 변환하는 **Builder Experience**다. Agent Stage Runner는 생성 중인 에이전트를 실행·시험하는 **Build and Validation Plane**이다. Tool Gateway는 회사가 허용한 업무 도구를 제공하는 **Tool Plane**이다. Agent Lifecycle은 검증된 결과를 버전 있는 조직 자산으로 관리하는 **Control Plane**이다.
